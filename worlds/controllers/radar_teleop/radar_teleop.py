...
class Radviz:
    ...
    def sense(self):
        ...
        self.actuationprocessing = Dataprocessing(max_length = 4)
    ...
    def distance_policy(self, lateral_velocity, lateral_distance, speed):
        ...
        if policy > 0:
            ...
            bit = 1
        self.actuationprocessing.addqueue((self.current_time, bit))
        values = self.actuationprocessing.get_distances()
        times, y_values = zip(*values)
        self.ax3.plot(times, y_values, label="Actuation signal",
                      color="red")  # Subplot
